...
其实在前面章节中，讲到格式的时候笔者做了很多的删减。原版的谷歌风格指南规定了很多关于代码缩进，以及代码前后之间空格间距的要求，远不止 [chap 1.4](#chap-14-格式) 中的那点内容，手敲这些内容只会让你破口大骂。
...
> 嵌入式领域常用的 IDE 例如 Keil 似乎也支持，但笔者也并未实践过。笔者非常不推荐使用 Keil 编写代码，具体原因过多，暂不列出。
...
既然 **clang-format** 是属于 **clang** 工具链中的一个独立工具，属于 **clang** 生态的一部分，那么最好就是直接使用 Vscode 中的 **clang** 插件。 

在安装好 **clang** 插件后，直接使用快捷键 `shift+alt+f` 即可对 C/C++ 代码进行格式化整理。
